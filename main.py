...
POLY_BOTTOM_WIDTH = 2.0  # 이미지 하단의 ROI 너비 비율
POLY_TOP_WIDTH = 0.4     # 이미지 상단의 ROI 너비 비율
POLY_HEIGHT = 0.3        # 이미지에서 ROI의 높이 비율

# 흰색 필터링 함수 (차선이 흰색일 경우 유용)
def filter_white(img):
    """흰색 영역만 추출하는 필터링 함수"""
    lower = np.array([150, 150, 150])  # BGR 흰색 하한
...
def main():
    ...
    try:
        while True:
            ...
            edges = cv2.Canny(gray, 30, 100)  # 엣지 검출
